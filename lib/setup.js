...
async function createGeneralPaths(cwd) {
  console.log(chalk.white('Creating directories'))

  await mkDirOrSkip(path.resolve(cwd, 'cut-version'))
  await mkDirOrSkip(path.resolve(cwd, 'ansible'))
  await mkDirOrSkip(path.resolve(cwd, 'gantry'))
  await mkDirOrSkip(path.resolve(cwd, 'gantry/source'))
  await mkDirOrSkip(path.resolve(cwd, 'gantry/run'))
  await mkDirOrSkip(path.resolve(cwd, 'gantry/run/dataf'))
  await mkDirOrSkip(path.resolve(cwd, 'gantry/run/dataf/cs'))
  await mkDirOrSkip(path.resolve(cwd, 'gantry/run/dataf/cs/1'))
}
