...
async function CreateContentForFile(
  ...
  if (FileName.indexOf('[application]') !== -1) {
    // If the file has [isSite] in its name, remeber that and remove [isSite] from name
    const optionIsSite = FileName.indexOf('[isSite]') !== -1
    if (optionIsSite) {
      FileName = FileName.replace('[isSite]', '')
    }

    for (let applicationName in rebarProject.applications) {
      const application = rebarProject.applications[applicationName]

      // If the file has [isSite] in its name, and the application is not a site, skip
      if (optionIsSite && !application.isSite) continue
