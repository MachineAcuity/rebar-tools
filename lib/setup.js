...
async function CreateProjectContentForFile( rebarProject, sourceDir, destDir, FileName){
  const templateContents = (await readFileAsync( path.join( sourceDir, FileName ))).toString()
  ...
  if( FileName.indexOf('[application]') !== -1)
    for( let application in rebarProject.applications){
      const destinationFileName = path.join(destDir, FileName.replace('[application]', application))
      promises.push( ensureFileContent(
        destinationFileName,
        null,
        template({
          rebarProject,
...
async function CreateProjectContentForDirectory( rebarProject, cwd, projectSourceDirName, directory){
...
async function createProjectContent(rebarProject, cwd){
  ...
  await CreateProjectContentForDirectory(rebarProject, cwd, projectSourceDirName, projectSourceDir)
}
