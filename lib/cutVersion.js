...
function parseVersionString(versionAsString, digitsExpected) {
  ...
  for (let ix = 0; ix < arrVersion.length; ix++) arrVersion[ix] = Number(arrVersion[ix])
  ...
  if (
    ...
    throw new Error(
      '💔  Version should have four numeric components. Instead found: ' + versionAsString,
    )

  return arrVersion
}

async function getPackageJSVersion(cwd, repositoryName) {
  const packageJSFileName = path.join(cwd, '/cut-version', repositoryName, '/units/_configuration/package.js')

  try {
    const packageJS = await readFileAsync(
      packageJSFileName,
      'utf8',
    )
    ...
    console.log(
      '💔  Failed to parse version in ' + packageJSFileName,
    )
    throw new Error(ex)
  }
}

async function getPackageJSONVersion(cwd, repositoryName) {
  const packageJSONName = path.join(cwd, 'cut-version/', repositoryName, 'package.json')

  try {
    const packageJSONAsString = await readFileAsync(
      packageJSONName,
      'utf8',
    )
...
async function bumpVersion(cwd, repository) {
  ...
  console.log('🔥  Current version in package.js:   ' + arrPackageJSVersion.join('.'))
  console.log('🔥  Current version in package.json: ' + arrPackageJSONVersion.join('.'))

  // Use package.json version for package.js (could have been updated - reset build number then)
  if( arrPackageJSVersion[0] != arrPackageJSONVersion[0] ||
...
async function createReleaseBranch(cwd, repositoryName, repositoryRevision) {
  const repository = await nodegit.Repository.open('./cut-version/' + repositoryName)
  ...
  const repoPath = path.join(cwd, 'cut-version', repositoryName)
...
async function ensureRepositoryIsOnDevelop(cwd, repositoryName) {
  const repoPath = path.join(cwd, 'cut-version', repositoryName)
  ...
  if (branchName != 'develop') {
    throw new Error(
      '💔  Cutting new versions should be done on develop, current branch is: ' + branchName,
    )
  }
}

function buildVersion(cwd, repositoryName) {
  const repoPath = path.join(cwd, 'cut-version', repositoryName)
  ...
  console.log(
    execSync(
      'cd ' + repoPath + ' && rm -rf deployment && mkdir deployment && npm run build-relay',
    ).toString(),
  )

  console.log('🐢  Running build server .... ')
  console.log(execSync('cd ' + repoPath + ' && npm run build-server').toString())

  console.log('🐢  Running build webpack .... ')
  console.log(execSync('cd ' + repoPath + ' && npm run build-webpack ').toString())
}

// async function cloneRepositoryUsingNodeGit( repositoryName, repositoryUrl ) {
//
//   const repositoryPath = path.join( process.cwd(), './cut-version/', repositoryName )
//
...
async function cloneRepository(cwd, repositoryName, repositoryUrl) {
  const repositoryParentPath = path.join(cwd, './cut-version/')
  const repositoryPath = path.join(repositoryParentPath, repositoryName)
...
module.exports.cutVersion = async (cwd, repositoryName, repositoryShortName, repositoryUrl) => {

  // Get develop branch from github
  await cloneRepository(cwd, repositoryName, repositoryUrl)

  // Verify that repo is on develop. If any poart of cloning failed, this will fail also
  await ensureRepositoryIsOnDevelop(cwd, repositoryName)

  // Determine the version number and update
  const repositoryRevision = await bumpVersion(cwd, repositoryName)

  // Build version
  buildVersion(cwd, repositoryName)

  // Create release branch
  await createReleaseBranch(repositoryName, repositoryRevision)
  ...
  console.log('  ./update-' + repositoryShortName + ' ' + repositoryRevisionString)
  console.log('\n')
  console.log('💡  To release after testing:\n')
  console.log('  ./publish-' + repositoryShortName + ' ' + repositoryRevisionString)
  console.log('\n')
}
