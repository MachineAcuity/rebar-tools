...
async function createReleaseBranch_async(
  ...
  await executeExternal(
    ...
    [
      'merge',
      '-m',
      '"Merge master to develop with no fast forward by rebar tools"',
      '--no-ff',
      'master',
    ],
    { cwd: repoPath },
...
async function buildVersion_async(cwd, applicationName) {
  const repoPath = path.join(cwd, 'cut-version', applicationName)
...
module.exports.cutVersion = async (cwd, applicationName, repositoryUrl) => {
  ...
  // Build version
  await buildVersion_async(cwd, applicationName)

  // Create release branch
  await createReleaseBranch_async(cwd, applicationName, repositoryRevision)
