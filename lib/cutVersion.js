...
async function getPackageJSVersion(cwd, applicationName) {
  ...
  try {
    ...
    console.error(
      chalk.red('💔  Failed to parse version in ') +
        chalk.bold.green(packageJSFileName),
    )
    throw new Error(ex)
  }
}
...
async function bumpVersion(cwd, repository) {
  ...
  console.log(
    chalk.white('🔥  Current version in package.js:   ') +
      chalk.bold.green(arrPackageJSVersion.join('.')),
  )
  console.log(
  ...
  console.log(
    chalk.white('🔥  New version in package.js:       ') +
      chalk.bold.green(newPackageJSVersion),
  )
...
module.exports.cutVersion = async (cwd, applicationName, repositoryUrl) => {
  ...
  console.log(chalk.gray('💡  To test with gantry:\n'))
  console.log(
