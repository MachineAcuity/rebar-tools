...
async function getPackageJSVersion(cwd, applicationName) {
  const packageJSFileName = path.join(cwd, '/cut-version', applicationName, '/units/_configuration/package.js')
...
async function getPackageJSONVersion(cwd, applicationName) {
  const packageJSONName = path.join(cwd, 'cut-version/', applicationName, 'package.json')
...
async function createReleaseBranch(cwd, applicationName, repositoryRevision) {
  const repository = await nodegit.Repository.open('./cut-version/' + applicationName)
...
module.exports.cutVersion = async (cwd, applicationName, repositoryUrl) => {
  ...
  console.log('  ./publish-' + applicationName + ' ' + repositoryRevisionString)
  console.log('\n')
}
