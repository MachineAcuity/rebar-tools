...
async function loadRebarProject() {
  ...
  for (let lastCwd = '!'; process.cwd() !== lastCwd; process.chdir('..')) {
    ...
    try {
      rebarProjectAsBuffer = await fs.readFile('tools/rebar-project.json')
      break
    } catch (err) {
  ...
  if (rebarProjectAsBuffer == null)
    throw new Error('Could not find tools/rebar-project.json')

  const rebarProjectAsString = rebarProjectAsBuffer.toString()

  failureDescription = 'File tools/rebar-project.json is not in JSON format'
  const rebarProject = JSON.parse(rebarProjectAsString)
...
async function commandSetup(argv) {
  try {
    ...
    await createGeneralPaths(rebarProject, cwd)
